...
if len(wo_lbl_lines)!=len(w_lbl_lines):
	print "Error:: number of lines incorrect"
	exit(1)
...
for i in range(len(wo_lbl_lines)):
	...
	if w_items[1]!="L" and w_items[1]!="P" and w_items[1]!="LA" and w_items[1]!="C" and w_items[1]!="N" and w_items[1]!="T" and w_items[1]!="A" and w_items[1]!="O":
		print "Error:: label token is different from Locality (L), Total Price (P), Land Area (LA), Cost per land area (C), Contact name (N), Contact telephone (T), Attributes of the property (A), Other (O) on line %d" %(i+1)
		exit(1)
